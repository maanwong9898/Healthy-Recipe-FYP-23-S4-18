...
public interface RecipeRepository extends JpaRepository<Recipe, Long> {
    ...
    @Query(value =  getDistinctDTOQuery  +" WHERE r.dietaryPreferencesId = :dp AND r.id NOT IN (SELECT DISTINCT re.id FROM Recipe re JOIN re.allergies al WHERE al.id IN (:allergies))")
    ...
    @Query(value = getDistinctDTOQuery + " WHERE r.id NOT IN (SELECT DISTINCT re.id FROM Recipe re JOIN re.allergies al WHERE al.id IN (:allergies))")
