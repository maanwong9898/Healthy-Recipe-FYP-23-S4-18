...
public interface RecipeRepository extends JpaRepository<Recipe, Long> {
    ...
    @Query(value =  getDTOQuery + " WHERE r.dietaryPreferencesId = :dp AND r.active = TRUE")
    ...
    @Query(value =  getDistinctDTOQuery  +" WHERE r.active = TRUE AND r.dietaryPreferencesId = :dp AND r.id NOT IN (SELECT DISTINCT re.id FROM Recipe re JOIN re.allergies al WHERE al.id IN (:allergies))")
    ...
    @Query(value = getDistinctDTOQuery + " WHERE r.active = TRUE AND r.id NOT IN (SELECT DISTINCT re.id FROM Recipe re JOIN re.allergies al WHERE al.id IN (:allergies))")
    ...
    @Query(value= getDistinctDTOQuery + " WHERE r.id NOT IN (:ids) AND r.active = TRUE ORDER BY r.createdDT LIMIT :count")
    List<RecipeDTO> findLatestRecipe(@Param("ids") List<Long> ids,@Param("count") Integer count);
 
    @Query(value= getDistinctDTOQuery + " WHERE r.active = TRUE ORDER BY r.createdDT LIMIT :count")
