...
-- many to one
CREATE TABLE User_Info_Over_Time(  
	UserID 					VARCHAR(255) NOT NULL,
	Saved_Date			DATE NOT NULL DEFAULT(CURDATE()),
	Weight 		BIT(1) NOT NULL, 
    Info                DECIMAL(5,2) NOT NULL,
 	CONSTRAINT ID_PKey PRIMARY KEY (UserID, Saved_Date, Weight),
	CONSTRAINT Info_ID_FKey FOREIGN KEY (UserID) REFERENCES USERACCOUNT (ID)
);
